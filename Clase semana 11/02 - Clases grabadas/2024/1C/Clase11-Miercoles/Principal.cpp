...
public:
	...
	Mensaje* buscarMensajeMasVotadoDelUsuarioSegunTematica(
		...
		//unsigned int mayor = -1; NOOOOOOOOOOO
		foros->iniciarCursor();
		while(foros->avanzarCursor()) {
			Foro * foro = foros->obtenerCursor();
			if (this->tieneLaTematica(foro->obtenerTematicas(), tematicaBuscada)) {
				Lista<Mensaje *> * mensajes = this->buscarMensajesDeUsuario(foro, usuarioBuscado);
				resultado = this->buscarMasVotado(resultado, mensajes);
			}
		}
		return resultado;
	}
